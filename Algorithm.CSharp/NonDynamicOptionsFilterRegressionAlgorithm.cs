        ...
        public override void OnSecuritiesChanged(SecurityChanges changes)
        {
            if (++_securitiesChangedCount < 3)
            {
                ...
                if (_securitiesChangedCount == 1)
                {
                    ...
                    if (underlying == null)
                    {
                        throw new Exception("Unexpected security changes: on the first OnSecuritiesChanged callback, we expect the underlying to be added.");
                    }
                }
                ...
                if (changes.AddedSecurities.Count <= 1)
                {
                    throw new Exception("Unexpected security changes count: " +
                        "on second and second OnSecuritiesChanged callbacks we expect options to be added");
                }
